...
const thirdPromise = new Promise((resolve)=>{
    const clicks = [];
    function clicker(e) {
        if (clicks.length < 2 && !clicks.includes(e.type)) clicks.push(e.type);
        else {
            clicks.length = 0;
            clicks.push(e.type);
        }
        if (clicks.length === 2) resolve("Third promise was resolved");
    }
    document.addEventListener("click", clicker);
    document.addEventListener("contextmenu", clicker);
});
function success(result) {
    document.body.insertAdjacentHTML("afterbegin", '<div data-qa="notification">');
